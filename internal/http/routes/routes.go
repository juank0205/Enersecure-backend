...
func SetupRoutes(db *db.DB, rootMux *http.ServeMux) {
	...
	//
	rootMux.Handle("/api/public/", http.StripPrefix("/api/public", publicStack(publicMux)))
	rootMux.Handle("/api/private/", http.StripPrefix("/api/private", privateStack(privateMux)))
}
